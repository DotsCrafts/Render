...
async function ensureAttached(tabId, aggressiveRetry = false) {
  ...
  if (lastError) {
    ...
    console.warn(
      `[opencli] attach failed for tab ${tabId}: url=${finalUrl}, windowId=${finalWindowId}, error=${lastError}`,
    );
...
async function screenshot(tabId, options = {}) {
  ...
  const overrideWidth =
    options.width && options.width > 0 ? Math.ceil(options.width) : void 0;
  const overrideHeight =
    !fullPage && options.height && options.height > 0
      ? Math.ceil(options.height)
      : void 0;
  const needsOverride =
    fullPage || overrideWidth !== void 0 || overrideHeight !== void 0;
  if (needsOverride) {
...
function matchesDownloadPattern(item, pattern) {
  ...
  const haystack = [item.filename, item.url, item.finalUrl, item.mime]
    .filter(Boolean)
    .join("\n")
    .toLowerCase();
  ...
  return await new Promise((resolve) => {
    ...
    const inspectById = async (id) => {
      ...
      if (item.state === "complete" || item.state === "interrupted")
        finish(downloadResult(item, startedAt));
    };
    ...
    void chrome.downloads
      .search({
        ...
        startedAfter: new Date(
          startedAt - Math.max(timeout, 1e3),
        ).toISOString(),
      })
      .then((recent) => {
        if (done) return;
        const completed = recent.find(
          (item) =>
            item.state === "complete" && matchesDownloadPattern(item, pattern),
        );
        if (completed) {
        ...
        for (const item of recent) {
          if (
            ...
            matchesDownloadPattern(item, pattern)
          )
            inProgressIds.add(item.id);
        }
      })
...
async function ensureFrameTarget(
  ...
  await chrome.debugger
    .sendCommand({ tabId }, "Target.setDiscoverTargets", { discover: true })
    .catch(() => {});
  await chrome.debugger
...
async function resolveFrameTargetId(tabId, frameId, targetUrl) {
  const result = await chrome.debugger
    .sendCommand({ tabId }, "Target.getTargets")
    .catch(() => null);
  ...
  const frameTarget = targets.find((candidate) => {
    ...
    return (
      ...
      (candidateId === frameId || (!!targetUrl && candidate.url === targetUrl))
    );
  });
  ...
  const candidates = targets
    ...
    .map(
      (target) => `${target.targetId || target.id || "?"} ${target.url || ""}`,
    )
    .join("; ");
  throw new Error(
...
async function sendCommandInFrameTarget(
  tabId,
  frameId,
  method,
  params = {},
  ...
  const targetId = await ensureFrameTarget(
    tabId,
    frameId,
    aggressiveRetry,
    targetUrl,
...
function registerFrameTracking() {
  ...
  chrome.debugger.onEvent.addListener((source, method, params) => {
    ...
    if (method === "Runtime.executionContextCreated") {
      ...
      if (!context?.auxData?.frameId || context.auxData.isDefault !== true)
        return;
...
async function evaluateInFrame(
  ...
  if (contextId === void 0) {
    await sendCommandInFrameTarget(
      tabId,
      frameId,
      "Runtime.enable",
      {},
    ...
    const result2 = await sendCommandInFrameTarget(
      tabId,
      frameId,
      "Runtime.evaluate",
      {
    ...
    if (result2.exceptionDetails) {
      const errMsg =
        result2.exceptionDetails.exception?.description ||
        result2.exceptionDetails.text ||
        "Eval error";
      throw new Error(errMsg);
    }
  ...
  chrome.debugger.onEvent.addListener(async (source, method, params) => {
    ...
    if (method === "Network.requestWillBeSent") {
      ...
      entry.requestBodyKind = request?.hasPostData ? "string" : "empty";
      {
        ...
        entry.requestBodyPreview = truncated
          ? raw.slice(0, CDP_REQUEST_BODY_CAPTURE_LIMIT)
          : raw;
      ...
      try {
        ...
        if (typeof body?.body === "string") {
          ...
          const stored = truncated
            ? body.body.slice(0, CDP_RESPONSE_BODY_CAPTURE_LIMIT)
            : body.body;
...
async function resolveTargetId(tabId) {
  ...
  if (!result)
    throw new Error(`No targetId for tab ${tabId} — page may have been closed`);
  return result;
}
async function resolveTabId$1(targetId) {
  ...
  if (result === void 0)
    throw new Error(`Page not found: ${targetId} — stale page identity`);
  return result;
}
...
function getSurfaceFromKey(key) {
  return key.split(LEASE_KEY_SEPARATOR, 1)[0] === "adapter"
    ? "adapter"
    : "browser";
}
...
function getIdleTimeout(key) {
  ...
  const adapterPersistent =
    getSurfaceFromKey(key) === "adapter" &&
    (session?.lifecycle === "persistent" ||
      sessionLifecycleOverrides.get(key) === "persistent");
...
function getWindowMode(key) {
  return (
    sessionWindowModeOverrides.get(key) ??
...
async function readRegistry() {
  try {
    ...
    if (!stored || stored.version !== 2 || typeof stored.leases !== "object")
      return emptyRegistry();
    const storedContainers =
      stored.ownedContainers && typeof stored.ownedContainers === "object"
    ...
    return {
      ...
      ownedContainers: {
        interactive: {
          windowId:
            typeof storedContainers.interactive?.windowId === "number"
              ? storedContainers.interactive.windowId
              : null,
          groupId:
            typeof storedContainers.interactive?.groupId === "number"
              ? storedContainers.interactive.groupId
              : null,
        },
        automation: {
          windowId:
            typeof storedContainers.automation?.windowId === "number"
              ? storedContainers.automation.windowId
              : null,
          groupId:
            typeof storedContainers.automation?.groupId === "number"
              ? storedContainers.automation.groupId
              : null,
        },
      },
...
async function collectOwnedGroupCandidates(role) {
  ...
  for (const [leaseKey, session] of automationSessions.entries()) {
    if (
      !session.owned ||
      getOwnedWindowRole(leaseKey) !== role ||
  ...
  if (ownedPreferredTabIds.size > 0) {
    try {
      ...
      for (const group of allGroups) {
        ...
        if (
          tabsInGroup.some(
            (tab) => tab.id !== void 0 && ownedPreferredTabIds.has(tab.id),
          )
        ) {
          groupsById.set(group.id, group);
        }
  ...
  const candidates = await Promise.all(
    [...groupsById.values()].map(toOwnedContainerGroupCandidate),
  );
...
async function attachTabsToOwnedGroup(role, group, ids) {
  ...
  const missing = tabs
    .filter(
      (tab) => tab !== null && tab.id !== void 0 && tab.groupId !== group.id,
    )
    .map((tab) => tab.id);
  if (missing.length > 0)
    await chrome.tabs.group({ groupId: group.id, tabIds: missing });
  updateOwnedSessionWindowForTabs(role, ids, group.windowId);
  return group;
}
async function createOwnedGroup(role, windowId, ids) {
  if (ids.length === 0)
    throw new Error(`Cannot create ${role} tab group without tabs`);
...
async function ensureOwnedContainerGroup(role, fallbackWindowId, tabIds) {
  ...
  const trackedGroupPromise = nextGroupPromise.finally(() => {
    if (container.groupPromise === trackedGroupPromise)
...
async function ensureOwnedContainerGroupUnlocked(role, fallbackWindowId, ids) {
  try {
    ...
    let canonical = selected
      ...
      : null;
    if (canonical) {
      canonical = await convergeOwnedGroupDuplicates(
        role,
        canonical,
        candidates,
      );
    ...
    console.warn(
      `[opencli] Failed to ensure ${role} tab group: ${err instanceof Error ? err.message : String(err)}`,
    );
    throw err;
  }
}
async function ensureOwnedContainerWindow(
  ...
  container.promise = ensureOwnedContainerWindowUnlocked(
    role,
    initialUrl,
    mode,
  ).finally(() => {
...
async function ensureOwnedContainerWindowUnlocked(
  ...
  if (container.windowId !== null) {
    try {
      ...
      if (group2) {
        ...
        const initialTabId3 = await findReusableOwnedContainerTab(
          group2.windowId,
          group2.id,
        );
        return {
      ...
      const initialTabId2 = await findReusableOwnedContainerTab(
        container.windowId,
        null,
      );
  ...
  const startUrl =
    initialUrl && isSafeNavigationUrl(initialUrl) ? initialUrl : BLANK_PAGE;
  ...
  await persistRuntimeState();
  console.log(
    `[opencli] Created owned ${role} window ${container.windowId} (start=${startUrl})`,
  );
...
async function createOwnedTabLease(leaseKey, initialUrl) {
  return withLeaseMutation(() =>
    createOwnedTabLeaseUnlocked(leaseKey, initialUrl),
  );
}
async function createOwnedTabLeaseUnlocked(leaseKey, initialUrl) {
  ...
  const { windowId, initialTabId } = await ensureOwnedContainerWindow(
    role,
    targetUrl,
    getWindowMode(leaseKey),
  );
  let tab;
  ...
  const tabId = tab.id;
  if (!tabId)
    throw new Error("Failed to create tab lease in automation container");
...
async function getAutomationWindow(leaseKey, initialUrl) {
  ...
  const role = getOwnedWindowRole(leaseKey);
  return (
    await ensureOwnedContainerWindow(role, initialUrl, getWindowMode(leaseKey))
  ).windowId;
}
chrome.windows.onRemoved.addListener(async (windowId) => {
  ...
  for (const [leaseKey, session] of automationSessions.entries()) {
    if (session.windowId === windowId) {
      console.log(
        `[opencli] ${session.surface} container closed (session=${session.session})`,
      );
      if (session.idleTimer) clearTimeout(session.idleTimer);
      automationSessions.delete(leaseKey);
      sessionTimeoutOverrides.delete(leaseKey);
...
chrome.tabs.onRemoved.addListener(async (tabId) => {
  ...
  for (const [leaseKey, session] of automationSessions.entries()) {
    if (session.preferredTabId === tabId) {
      ...
      scheduleIdleAlarm(leaseKey, IDLE_TIMEOUT_NONE);
      console.log(
        `[opencli] Session ${session.session} detached from tab ${tabId} (tab closed)`,
      );
    }
  }
  await persistRuntimeState();
...
async function handleCommand(cmd) {
  ...
  if (
    surface === "adapter" &&
    (cmd.siteSession === "persistent" || cmd.siteSession === "ephemeral")
  ) {
  ...
  try {
    ...
    return {
      ...
      ...(err instanceof CommandFailure && err.hint
        ? { errorHint: err.hint }
        : {}),
    };
  }
...
async function resolveTab(tabId, leaseKey, initialUrl) {
  ...
  if (tabId !== void 0) {
    try {
      ...
      if (
        ...
        console.warn(
          `[opencli] Tab ${tabId} drifted to window ${tab.windowId}, moving back to ${session.windowId}`,
        );
        try {
          ...
          if (
            moved.windowId === session.windowId &&
            isDebuggableUrl(moved.url)
          ) {
  ...
  if (
    !existingSession ||
    (existingSession.owned && existingSession.preferredTabId === null)
  ) {
    return createOwnedTabLease(leaseKey, initialUrl);
  }
  const windowId = await getAutomationWindow(leaseKey, initialUrl);
  const role = getOwnedWindowRole(leaseKey);
  const group = existingSession?.owned
  ...
  if (reuseTab?.id) {
    ...
    try {
      ...
      if (isDebuggableUrl(updated.url))
        return { tabId: reuseTab.id, tab: updated };
      console.warn(
  ...
  if (!newTab.id)
    throw new Error("Failed to create tab in automation container");
...
async function handleExec(cmd, leaseKey) {
  ...
  try {
    ...
    if (cmd.frameIndex != null) {
      ...
      const data2 = await evaluateInFrame(
        tabId,
        cmd.code,
        frames[cmd.frameIndex].frameId,
        aggressive,
      );
...
async function handleNavigate(cmd, leaseKey) {
  ...
  await new Promise((resolve) => {
    ...
    const listener = (id, info, tab2) => {
      ...
      if (
        ...
        isNavigationDone(tab2.url ?? info.url)
      ) {
        finish();
      }
    };
  ...
  if (
    ...
    console.warn(
      `[opencli] Tab ${tabId} drifted to window ${tab.windowId} during navigation, moving back to ${postNavigationSession.windowId}`,
    );
    try {
      await chrome.tabs.move(tabId, {
...
async function handleTabs(cmd, leaseKey) {
  ...
  if (session && !session.owned && cmd.op !== "list") {
    return {
      ...
      errorHint:
        "Unbind the session first, or use a different session for owned OpenCLI tabs.",
    };
  }
  switch (cmd.op) {
    case "list": {
    ...
    case "new": {
      ...
      const tabId = tab.id;
      if (!tabId)
        return { id: cmd.id, ok: false, error: "Failed to create tab" };
      const group = await ensureOwnedContainerGroup(
        getOwnedWindowRole(leaseKey),
        windowId,
        [tabId],
      );
      const sessionWindowId = group?.windowId ?? tab.windowId;
    ...
    case "close": {
      if (cmd.index !== void 0) {
        ...
        const closedPage2 = await resolveTargetId(target.id).catch(
          () => void 0,
        );
...
async function handleCdp(cmd, leaseKey) {
  ...
  try {
    ...
    const routeFrameId =
      typeof params.frameId === "string" && params.sessionId === "target"
        ? params.frameId
        : void 0;
    const routeTargetUrl =
      typeof params.targetUrl === "string" ? params.targetUrl : void 0;
    const data = routeFrameId
      ? await sendCommandInFrameTarget(
          tabId,
          routeFrameId,
          cmd.cdpMethod,
          stripOpenCliFrameRoutingParams(params, true),
          aggressive,
          3e4,
          routeTargetUrl,
        )
...
async function handleCloseWindow(cmd, leaseKey) {
  const sessionName =
    automationSessions.get(leaseKey)?.session ?? getSessionFromKey(leaseKey);
  ...
  if (session.owned) {
    ...
    if (tabId !== null) {
      ...
      if (hasOtherOwnedLease) {
        ...
        console.log(
          `[opencli] Released owned tab lease ${tabId} (session=${session.session}, surface=${session.surface}, ${reason})`,
        );
      } else {
        try {
          ...
          console.log(
            `[opencli] Released owned tab lease ${tabId} as reusable placeholder (session=${session.session}, surface=${session.surface}, ${reason})`,
          );
        } catch {
          await chrome.tabs.remove(tabId).catch(() => {});
          console.log(
      ...
      console.log(
        `[opencli] Released legacy owned window lease ${session.windowId} without closing container (session=${session.session}, surface=${session.surface}, ${reason})`,
      );
    }
  } else if (session.preferredTabId !== null) {
    await safeDetach(session.preferredTabId);
    console.log(
      `[opencli] Detached borrowed tab lease ${session.preferredTabId} (session=${session.session}, surface=${session.surface}, ${reason})`,
    );
  }
  automationSessions.delete(leaseKey);
  sessionTimeoutOverrides.delete(leaseKey);
...
async function reconcileTargetLeaseRegistry() {
  ...
  for (const role of Object.keys(ownedContainers)) {
    ownedContainers[role].windowId =
      registry.ownedContainers[role]?.windowId ?? null;
    ownedContainers[role].groupId =
      registry.ownedContainers[role]?.groupId ?? null;
  ...
  for (const [leaseKey, stored] of Object.entries(registry.leases)) {
    ...
    try {
      ...
      if (
        stored.lifecycle === "ephemeral" ||
        stored.lifecycle === "persistent" ||
        stored.lifecycle === "pinned"
      ) {
      ...
      const session = makeSession(leaseKey, {
        session:
          typeof stored.session === "string"
            ? stored.session
            : getSessionFromKey(leaseKey),
        surface:
          stored.surface === "adapter"
            ? "adapter"
            : getSurfaceFromKey(leaseKey),
        kind:
          stored.kind === "bound" || stored.owned === false ? "bound" : "owned",
      ...
      if (session.owned) {
        const role = getOwnedWindowRole(leaseKey);
        if (ownedContainers[role].windowId === null)
          ownedContainers[role].windowId = tab.windowId;
        const group = await ensureOwnedContainerGroup(role, tab.windowId, [
          tabId,
        ]);
        if (group) {
...
async function handleBind(cmd, leaseKey) {
  ...
  const boundTab =
    activeTabs.find((tab) => isDebuggableUrl(tab.url)) ??
  ...
  if (
    current &&
    !current.owned &&
    current.preferredTabId !== null &&
    current.preferredTabId !== boundTab.id
  ) {
  ...
  resetWindowIdleTimer(leaseKey);
  console.log(
    `[opencli] Session ${getSessionFromKey(leaseKey)} explicitly bound to tab ${boundTab.id} (${boundTab.url})`,
  );
